...
public class PricePlanComparatorController {
    ...
    @Operation(summary = "Add a new price plan", description = "This API adds a new price plan to the system.")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "201",
                        description = "Price plan added successfully",
                        content = {@Content(mediaType = "application/json")}),
                @ApiResponse(
                        responseCode = "400",
                        description = "Invalid price plan data provided",
                        content = {@Content(mediaType = "application/json")}),
                @ApiResponse(
                        responseCode = "500",
                        description = "Internal server error",
    ...
    @PostMapping("/add-price-plan")
    public ResponseEntity<String> addPricePlan(@RequestBody PricePlan pricePlan) {
        pricePlanServiceImpl.addPricePlan(pricePlan);
        URI location = URI.create("/price-plans/v1/add-price-plan/" + pricePlan);

        return ResponseEntity.created(location).body("Price plan added successfully");
    }
}
